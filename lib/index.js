...
export const Config = Schema.object({
    ...
    enableSound: Schema.boolean().default(true).description('Play synthesized audio chime on completion, error, or approval'),
    enableToasts: Schema.boolean().default(true).description('Show in-app on-screen toast notifications across sessions'),
    enableDesktopNotifications: Schema.boolean().default(true).description('Show native desktop/OS push notifications (Windows, macOS, Linux)'),
...
export async function resolveWebhookValue(ctx, refOrUrl) {
    ...
    if (ctx?.credentials && typeof ctx.credentials.resolve === 'function') {
        try {
            ...
            console.warn(`[plugin-notify] credential resolve skipped for ${v}: ${String(error)}`);
        }
    }
