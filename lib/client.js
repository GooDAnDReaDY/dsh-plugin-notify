window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    function handleIncomingEvent(event, ctx, t, getSettings) {
      ...
      // 1. Audio chime
      if (cfg.enableSound !== false) {
        playChime(event.kind)
      }

      // 2. In-app toast
      if (cfg.enableToasts !== false) {
      ...
      React.useEffect(() => {
        if (status === 'ready' && draft === null) {
          ...
          setDraft({
            ...
            enableToasts: stored.enableToasts !== false,
            enableDesktopNotifications: stored.enableDesktopNotifications !== false,
